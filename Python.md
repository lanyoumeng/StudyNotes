...
字符串的截取的语法格式如下：变量[头下标:尾下标:步长] 截取到尾下标-1
...
2. **`isinstance` 函数：**
   - `isinstance(obj, classinfo)` 返回一个布尔值，表示对象 `obj` 是否是 `classinfo` 类型或其子类的实例。
   - 例如：`isinstance(5, int)` 返回 `True`，表示整数 `5` 是整数类型的实例。
...
1. **位置参数（Positional Arguments）**：
   - 位置参数是最常见的参数类型。
   - 当您调用函数时，参数按照在函数定义中的顺序进行传递。
   - 参数的值将按照顺序与函数中的形参一一对应。
   - 位置参数不需要在函数调用时明确指定参数名。

2. **关键字参数（Keywor Arguments）**：
   
   - 关键字参数允许您在函数调用时明确指定参数名。
   - 这样，您可以忽略参数的顺序，只需提供参数名和对应的值。
   - 关键字参数非常有用，特别是当函数有多个参数或您希望提高可读性时。
...
![image-20240114210911412](C:\Users\to'm\AppData\Roaming\Typora\typora-user-images\image-20240114210911412.png)
...
代表当前对象的地址，而 **self.__class__** 则指向类。
