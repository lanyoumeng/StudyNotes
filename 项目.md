...
1. publishVids::user_id 缓存设置

1. CI/CD
2. Flink cdc 同步

1.redis---mysql      mysql-->flink cdc-->kafka-->订阅处理到redis
...
在wsl2上运行，服务的IP地址要在wsl2中使用ip address查看，

比如服务暴露一个http://127.0.0.1:7271/metrics接口 ，prometheus收集时要填172.26.71.21:7271/metrics来指定wsl2，

因为此时Prometheus使用的是主机网络，不是wsl2的，在主机上要用wsl2的ip地址访问
...
|                                                              |                                                              |                                             |                                                              |
| Nginx                                                        | 反向代理和负载均衡器                                         |                                             |                                                              |
| APISIX(UI： [Dashboard](https://apisix.apache.org/zh/docs/dashboard/USER_GUIDE/)) | API 网关，专门用于对 API 流量进行管理、路由和流量控制 API 的动态路由、限流、鉴权、监控 | ✓                                           | 9091被Prometheus采集 dashboard （3001:3001）                 |
...
| Elasticsearch(UI:elastic), Logstash, Kibana                  | Logging日志工具                                              | ✓                                           |                                                              |
|                                                              |                                                              |                                             |                                                              |
| docker-compose                                               | 部署                                                         | ✓                                           |                                                              |
| K8s                                                          | 部署                                                         |                                             |                                                              |
|                                                              |                                                              |                                             |                                                              |
...
//如果要保证实时性 -----》》删除redis/更新（redis+过期时间） 

然后flinkcdc 监听  mysql到kafka，  消费卡夫卡删除对应redis

| 服务                                                  | 数据信息                                | 数据类型                 | **key**                                           | **value**                                                    | 创建时机                                                     | 更新（刷新过期时间）                                         | 删除                           |
| :---------------------------------------------------- | :-------------------------------------- | :----------------------- | :------------------------------------------------ | :----------------------------------------------------------- | :----------------------------------------------------------- | :----------------------------------------------------------- | :----------------------------- |
| user                                                  | userInfoAll（不应该有这个表）           | hash                     | userInfoAll::user_id                              | user_info 静态信息                                           | 查询为空                                                     |                                                              | 1.过期时间                     |
| count 较频繁，可以不设置过期时间，或加锁              | hash无对应mysql表                       | count::user_id           | count 计数信息                                    | 查询为空                                                     | 其他服务更新1.视频发布2.点赞3.关注                           | 1.过期时间                                                   |                                |
| user                                                  | hash                                    | user::user_id            | user信息                                          | 查询为空                                                     |                                                              | 1.过期时间2.mysql表更新/删除                                 |                                |
|                                                       |                                         |                          |                                                   |                                                              |                                                              |                                                              |                                |
| video                                                 | video_info视频获赞数和跟随数是rpc调用的 | hash                     | videoInfo::video_id                               | video_info 静态信息type Video struct {    Id       int64     AuthorId int64     Title    string     PlayUrl  string     CoverUrl string } | 查询为空                                                     |                                                              | 1.过期时间2.video表更新/删除   |
| 系统发布的所有视频 id 用于视频流                      | zset无对应mysql表                       | videoAll                 | score: video发布时间戳   float64 member: video_id |                                                              | 不过期，发布视频时插入                                       |                                                              |                                |
| 用户发布视频的 id 用于发布列表                        | set无对应mysql表                        | publishVids::user_id     | video_ids                                         | 查询为空                                                     | video表创建一条记录：添加对应key-member  video表不会更新 video表删除一条记录：移除对应key-member | 1.过期时间                                                   |                                |
|                                                       |                                         |                          |                                                   |                                                              |                                                              |                                                              |                                |
| **favor**较频繁，可以不设置过期时间，或加锁           | 1.一条点赞信息                          | string无对应mysql表      | `fav::<videoId>::<userId>`                        | true/false                                                   | 查询为空                                                     | favorite表创建/更新记录，操作为：点赞：变为true取消点赞：变为false | 1.过期时间2.favorite表删除记录 |
| 2.用户点赞视频列表size=用户的点赞数量                 | set无对应mysql表                        | `userFavList::<userId>`  | video_id                                          | 查询为空                                                     | favorite表创建/更新记录，操作为： 点赞操作 -->cnt+1取消点赞-->cnt-1 favorite表删除记录,点赞为true-->cnt-1 | 1.过期时间                                                   |                                |
| 3.视频的获赞数                                        | string无对应mysql表                     | `videoFavCnt::<videoId>` | int64                                             | 查询为空                                                     | 1.过期时间                                                   |                                                              |                                |
| 4.视频作者的获赞数TotalFavorited                      | userTotalFavorited::authorId            | int64                    | 查询为空                                          | 1.过期时间                                                   |                                                              |                                                              |                                |
|                                                       |                                         |                          |                                                   |                                                              |                                                              |                                                              |                                |
|                                                       | 点赞该视频的用户id列表                  | Set                      | `fav::<videoId>`                                  | `<userId>`                                                   |                                                              |                                                              |                                |
|                                                       | 用户点赞的视频列表                      | set                      | `fav::<userId>`                                   | `<videoId>`                                                  |                                                              |                                                              |                                |
|                                                       |                                         |                          |                                                   |                                                              |                                                              |                                                              |                                |
| **comment**                                           | 评论用户                                | zset                     | comment::video_id                                 | score: 评论发布时间戳 member: 序列化为json（字符串的话，分隔符会和content冲突）Id        UserId   Content   CreateDate | 查询为空                                                     | comment表创建一条记录：添加对应comment--member  comment表不会更新 comment表删除一条记录：移除对应comment--member | 1.过期时间                     |
|                                                       |                                         |                          |                                                   |                                                              |                                                              |                                                              |                                |
| relation                                              | 关注用户的id                            | set                      | follow::user_id                                   | 关注用户的ids                                                | 查询为空                                                     | relation表创建一条记录：添加对应key-member  relation表不会更新 relation表删除一条记录：移除对应key-member | 1.过期时间                     |
| 粉丝的id                                              | set                                     | follower::user_id        | 粉丝的ids                                         | 查询为空                                                     | 1.过期时间                                                   |                                                              |                                |
| 好友的id 通过求关注和粉丝的交集可以得到，不用特地缓存 | set                                     | friend::user_id          | 好友的ids                                         | 查询为空                                                     | 1.过期时间                                                   |                                                              |                                |
|                                                       |                                         |                          |                                                   |                                                              |                                                              |                                                              |                                |
| message                                               | 历史消息                                | zset                     | message::history::小user_id+大user_id             | score: 消息发布时间戳 member: 序列化为json：user_idtarget_idcontent | 查询为空                                                     | message表创建一条记录：添加对应key-member  message表不会更新 messaget表删除一条记录：移除对应key-member | 1.过期时间                     |
|                                                       |                                         |                          |                                                   |                                                              |                                                              |                                                              |                                |
|                                                       |                                         |                          |                                                   |                                                              |                                                              |                                                              |                                |
|                                                       |                                         |                          |                                                   |                                                              |                                                              |                                                              |                                |
...
如果视频 ID 是整数或短字符串，可以通过合并存储多个 ID 以减少内存占用。虽然 Redis 没有原生支持这个功能，但你可以通过编码和解码来实现。
...
| 服务              | 数据信息           | 数据类型            | **key**                                  | **value**                                               | 缓存策略 |
| :---------------- | :----------------- | :------------------ | :--------------------------------------- | :------------------------------------------------------ | :------- |
| **user**          | user_info          | hash                | user_info::user_id                       | user_info 静态信息                                      | 旁路缓存 |
| **user**          | user_count         | hash                | user_count::user_id                      | user_count 计数信息                                     | 写回     |
| **interaction**   | 关注用户的id       | set                 | follow::user_id                          | 关注用户的ids                                           | 延迟双删 |
| **interaction**   | 粉丝的id           | set                 | fan::user_id                             | 粉丝的ids                                               | 延迟双删 |
| **interaction**   | 好友的id           | set                 | friend::user_id                          | 好友的ids                                               | 延迟双删 |
| **video**         | video_info         | hash                | video_info::video_id                     | video_info 静态信息+2个计数信息                         | 写回     |
| 系统发布的视频 id | zset               | video_publish::     | score: video发布时间戳  member: video_id |                                                         |          |
| 用户发布视频的 id | set                | user_video::user_id | video_ids                                | 旁路缓存                                                |          |
| **favor**         | 点赞视频的 id      | set                 | user_favorite::user_id                   | 标志位+video_id                                         | 写回     |
| **comment**       | 评论用户id+content | zset                | video_comment::video_id                  | score: 评论发布时间戳 member: id+user_id+content        | 延迟双删 |
| **chat**          | 历史消息           | zset                | chat::history::小user_id+大user_id       | score: 消息发布时间戳 member: user_id+target_id+content |          |
| **chat**          | 要持久化的消息     | zset                | chat::message::小user_id+大user_id       | score: 消息发布时间戳 member: user_id+target_id+content | 写回     |

1. **User 服务**
   1. 对 `user_info `进行缓存，使用 hash 数据类型，`user_info `属于静态信息，直接存入缓存并设置过期时间。
   2. 对 `user_count`进行缓存，使用 hash 数据类型，`user_count `属于读写相当的信息，业务代码中直接对缓存进行操作，采用定时任务将缓存持久化到数据库中。因为本项目中的，点赞功能对 `favorite_count`和`total_favorited `以及关注功能对 `follow_count `和 `follower_count `的计数非常频繁，使用 hash数据结构可以很方便的对缓存中的计数做出更改。
2. **Interaction 服务**
   1. 无论是 关注用户，粉丝，好友都用 set进行缓存，因为 关注操作 并不像点赞那样频繁，可以归类于读多写少的信息，采用 延迟双删的策略，延迟时间大于一次redis写入时间。
3. **Video 服务**
   1. 对 `video_info`缓存，使用 hash 数据类型，我并没有拆分 `video_info `中的静态信息和计数信息，当然可以选择拆分，因为有计数信息的存在，`favorite_count `和 `comment_count`，点赞属于读写相当，所以选择定时任务保证数据最终一致性。
   2. `publish_video`，根据需求使用 **zset缓存**，score是视频发布时间戳，member是视频id，因为需要按时间倒序取视频，使用zset很便捷，因为项目不涉及删除视频，所以该缓存不设置过期时间。很方便的获取到视频的id，然后根据视频Id去查询视频信息。
   3. `user_video`，使用 set存储用户发布的所有视频 id，方便查询，设置过期时间。
4. **favor 服务**
   1. 对 用户点赞的视频进行缓存，使用 set，因为操作频繁，所以业务代码直接操作缓存，用定时任务保证数据最终一致性。
5. **comment 服务**
   1. 对 视频下的评论进行缓存，使用 zset，根据需求 需要按时间戳按发布时间倒序返回 评论，属于读多写少，采用延迟双删策略。
6. **chat 服务**
   1. 对发送的消息设置两个缓存，一个是用来取历史记录的缓存，一个是用来将聊天记录定时持久化到数据库的缓存。
...
>   - 写策略：更新数据库的记录，删除缓存记录。
> - 写回：更新数据的时候，只更新缓存，同时将缓存数据设置为脏的，然后立马返回，并不会更新数据库。对于数据库的更新，会通过批量异步更新的方式进行。
> - 延迟双删：先删除缓存，再删除数据库，休眠一段时间，再删除缓存。
>
> 缓存过期时间：
>
> - 在一段时间基础上，再随机设置一段不长的时间段。二者相加作为缓存过期时间。
...
使用 Apache Kafka 处理视频流， 使用 RabbitMQ 处理用户活动和评论， 使用 WebSocket 提供实时通知等
...
当涉及异步处理和Kafka时，通常需要使用不同的语言和库来实现

。以下是一个用 Go 语言示例，展示了如何在后端接收视频文件并将视频信息异步发送到 Kafka：
...
advanceCommitL()：Raft 永远不会通过计算副本数目的方式去提交一个之前任期内的日志条目。只有领导人当前任期里的日志条目通过计算副本数目可以被提交；
...
1）、Git\etc\gitconfig ：Git 安装目录下的 gitconfig --system 系统级 2）、C:\Users\to'm.gitconfig 只适用于当前登录用户的配置 --global 全局

![image-20240709183557813](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/typora/202407091901786.png?token=AWX6LWSJ42PLUXU2DQ5BTHLGRUMMI)
...
![aHR0cHM6Ly9tbWJpei5xcGljLmNuL21tYml6X3BuZy91SkRBVUtyR0M3S3N1OFVsSVR3TWxiWDNrTUd0WjlwMHpRWlkzN3ExaWFHMG43NDQ1WDhZZ1BWdlpINUFxeUd2VDRSZ21veUljWmxKV2lhTGN4eURnU2RRLzY0MA](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/typora/202407092224721.png?token=AWX6LWV5FRJBLNCXEW5MDB3GRVEE6)
...
![aHR0cHM6Ly9tbWJpei5xcGljLmNuL21tYml6X3BuZy91SkRBVUtyR0M3S3N1OFVsSVR3TWxiWDNrTUd0WjlwME5KNEw5T1BJOWlhMU1taWJwdkRkNmNTZGRCZHZybGJkRXR5RU9yaDRDS25XVmlieWZDSGEzbHpYdy82NDA](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/typora/202407092224328.png?token=AWX6LWROUMNNDUEBY2FSA43GRVEFY)
...
![aHR0cHM6Ly9tbWJpei5xcGljLmNuL21tYml6X3BuZy91SkRBVUtyR0M3S3N1OFVsSVR3TWxiWDNrTUd0WjlwMGljejZYMmFpYklnVVd6SHh0d1g4a2ljUENLcERyc2lhUHpaazA0T2xJMmJ6bHlkemljQnVYVEp2TEVRLzY0MA](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/typora/202407092224570.png?token=AWX6LWSVMJWN3B4I343ZMDTGRVEHG)
...
![aHR0cHM6Ly9tbWJpei5xcGljLmNuL21tYml6X3BuZy91SkRBVUtyR0M3S3N1OFVsSVR3TWxiWDNrTUd0WjlwMDlpYU9obDBkQUNmTHJNd05iRHp1Y0dRMzBzM0huc2lhY3pmY1I2ZEM5T2VoaWN1d2liS3VIalJsemcvNjQw](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/typora/202407092225665.png?token=AWX6LWU42YFUOYEC2YL7UOLGRVEIK)
...
![aHR0cHM6Ly9tbWJpei5xcGljLmNuL21tYml6X3BuZy91SkRBVUtyR0M3S3N1OFVsSVR3TWxiWDNrTUd0WjlwMEFJSTZZVm9vVXppYnBpYnpKbm9PSEhYVXNMM2Y5RHFBNGhvclVpYmZjcEVaODhPeWYyZ1FRTlI2dy82NDA](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/typora/202407092225245.png?token=AWX6LWQT7ZBOJ2FX3KNO2BLGRVEJI)
...
1. `origin` 是远程仓库的别名，您可以根据需要自定义别名。
2. 将 `<remote_repository_url>` 替换为 GitHub 远程仓库的 URL。
3. 要查看Git远程仓库的信息，你可以使用以下命令：

```Plaintext
git remote -v
```

1. 这个命令会列出当前Git仓库配置的所有远程仓库，并显示它们的URL地址。通常，远程仓库的名称是 "origin"，这是Git默认的远程仓库名称。如果你有多个远程仓库，它们会依次列出。
2. 输出会类似于以下格式：
...
1. 其中，"(fetch)" 表示可以从远程仓库获取数据，"(push)" 表示可以向远程仓库推送数据。
2. 如果你想查看特定远程仓库的详细信息，可以使用以下命令：

```Plaintext
git remote show <remote_name>
```

1. 其中，`<remote_name>` 是远程仓库的名称，通常为 "origin"。这个命令会显示远程仓库的URL地址、分支信息以及其他相关配置。
2. 使用上述命令可以轻松查看你的Git仓库所关联的远程仓库信息。
...
![aHR0cHM6Ly9tbWJpei5xcGljLmNuL21tYml6X3BuZy91SkRBVUtyR0M3S3N1OFVsSVR3TWxiWDNrTUd0WjlwMEJPR3phRzRRVGM0SlhPMGhTbHdjTnR1ak56QXZ4ZWliU3JhakxZTENUNm90Tm5IRFY5eFlXd0EvNjQw](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/typora/202407092225857.png?token=AWX6LWUB6EPLSB5PUAYUQHLGRVEK6)

![aHR0cHM6Ly9tbWJpei5xcGljLmNuL21tYml6X3BuZy91SkRBVUtyR0M3S3N1OFVsSVR3TWxiWDNrTUd0WjlwMEF5bjg3d294ZmVwT2hTbFVqNEZRVEZVc2lhNGljMGo2YVF5NFR6MzJQUnVKMEhTVmVHZVV6VVJBLzY0MA](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/typora/202407092226910.png?token=AWX6LWRK5D5BS2BPG76HEQLGRVEL4)

IDEA中操作

![aHR0cHM6Ly9tbWJpei5xcGljLmNuL21tYml6X3BuZy91SkRBVUtyR0M3S3N1OFVsSVR3TWxiWDNrTUd0WjlwMHdITklZZVRIQzhhSEdBU29EeVpPNjRRaWNzbHFpYU1iMU9KMVoxTFBvaWMzTEJHeURJWUJhN1hYdy82NDA](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/typora/202407092226407.png?token=AWX6LWSIF76PMCICAQH4WFDGRVENC)
...
![img](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/typora/202407092231431.webp?token=AWX6LWVP7B3366L7DWJMVBTGRVE7G)
...
![img](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/typora/202407092228444.webp?token=AWX6LWXIEQ6IXGUCZX4D7QLGRVEUA)
...
![img](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/typora/202407092232435.webp?token=AWX6LWWYWLJ754XE4PDQ2Q3GRVFES)
...
![img](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/typora/202407092233031.webp?token=AWX6LWS6B3AU5FM3DZZNEWLGRVFHG)
...
![v2-a0f9e50e265f29d598ff03ecef8b1d2d_720w](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/typora/202407092234582.webp?token=AWX6LWQBIWL4HSLIZVWKUMLGRVFJW)

**倒树**：自上而下的层次结构，其中高级角色将其部分权限继承给下级角色。这种结构中层节点的权限均继承于底部节点，所以同层节点不存在共享权限。

![v2-9d591267a2c9c3f928b699bef1839850_720w](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/typora/202407092234404.webp?token=AWX6LWQAY3ZXSXZEXD6LUELGRVFKQ)

**网格：**自下而上和自上而下的组合，其中每个角色都可以从其下方和上方的节点继承权限。此种结构相对比较灵活，既可以有共享权限，也可以有自有权限，且顶级节点拥有最大的权限。

![v2-274533e79c4b64a40e631fed70e5b7a7_720w](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/typora/202407092234645.webp?token=AWX6LWTNKFT6ILCUAALHW33GRVFLI)
...
![v2-669402e6a5a59049d4629895ee5ced43_720w](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/typora/202407092234717.webp?token=AWX6LWXSYU3FF44234IN5LDGRVFL6)
...
![v2-8e2b114f376fac45fb8a1e6573ec74c6_180x120](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/typora/202407092234077.png?token=AWX6LWWDHV727JDJQKF7WFLGRVFM4)
...
![img](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/typora/202407092235673.webp?token=AWX6LWR5XV36OFKBBAKRSPDGRVFNU)
