...
7. publishVids::user_id 缓存设置

8. CI/CD

9. 视频 

   biz/videoKafkaMessage:&{3 回忆录2 ../../store/video/474bf94d-ae9f-45b1-b1d4-c2d14b02991b.mp4 474bf94d-ae9f-45b1-b1d4-c2d14b02991b.mp4}

10. 疑问为什么是../../  不是../../../

11. apisix的插件不能转换post请求，需要进行更改

12. cmd /c mklink "C:\Users\20588\AppData\Local\Packages\CanonicalGroupLimited.Ubuntu22.04LTS_79rhkp1fndgsc\LocalState\ext4.vhdx" "D:\Environment\wsl\ext4.vhdx"
...
2. 异步写入kafka时，ctx不要用上层传入的，  因为上层直接结束了，ctx相当于到期了
...
- **步骤：**
  ...
  3. 在 `AppendEntries` 中实现快速回滚以提高同步效率。
...
这些步骤将帮助你在多个阶段逐步实现 Raft 算法，最终构建一个容错的分布式系统。你可以按照实验的提示逐步实现和测试代码，确保每个阶段的功能都正常工作。
...
1. 7.6 是否一项提议只需要被多数派通过就可以提交？

2. 代码 readPersist的log类型

3. advanceCommitL()：Raft 永远不会通过计算副本数目的方式去提交一个之前任期内的日志条目。只有领导人当前任期里的日志条目通过计算副本数目可以被提交；

4. 心跳AppendEntries也要检查
...
1. 在实验2D中，测试人员定期调用 `Snapshot()` ,服务层在每个对等体上调用 `Snapshot()` （而不仅仅是在领导者上）。
2. 但是，现在可能会有一个follower远远落在后面，以至于leader已经丢弃了它需要赶上的日志条目; leader必须发送一个快照以及从快照开始的日志
...
#dtest
./dtest -p 4 -n 2 --output ./logs --verbose  --race TestInitialElection2A
...
VERBOSE=1 go test -run TestFigure8Unreliable2C | ./dslog -c 3 >logs/debug.log
VERBOSE=1 go test -run TestSnapshotInstall2D | ./dslog -c 5 >logs/debug.log
VERBOSE=1 go test -run TestFailNoAgree2B | ./dslog -c 5 -i LEAD,CLNT,TIMR
...
![202407092224570](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/docs/202407111557726.png)
...
![202407092225665](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/docs/202407111557218.png)
...
![QQ_1721047469278](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/docs/202407152044648.png)



如果误使用hard命令， reflog找到版本号，再使用reset回退

![QQ_1721047844452](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/docs/202407152051393.png)

#### 删除

![QQ_1721048572955](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/docs/202407152103051.png)
...
![QQ_1721047976230](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/docs/202407152052981.png)

![QQ_1721048365894](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/docs/202407152059986.png)

### 同类产品比较

![QQ_1721045902436](https://raw.githubusercontent.com/lanyoumeng/Drawing-bed/main/docs/202407152018459.png)
