...
5. rdb和rpc调用时，若用上层的ctx会超时，用context.Background()
...
8.  user、video服务....   docker build使用host模式，但容器却绑定的不是主机的端口，
...
**一个Join等操作 是由client客户端发起，在server服务端开始，经过raft达成一致和持久化，最后回到server服务端执行**
